...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($studentCheckResult->num_rows == 0) {
        ...
        if ($existingResult->num_rows > 0) {
            displayError("Record already exists. You cannot add the same grade for the same student, faculty, and course.");
        } else {
            // All checks passed, so insert the grade
            $query = "INSERT INTO grade (studentID, facultyID, courseID, gradeValue, date) VALUES ('$studentID', '$facultyID', '$courseID', '$gradeValue', '$date')";
