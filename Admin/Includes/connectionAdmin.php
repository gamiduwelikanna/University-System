...
$validUsernames = array("ad1", "ad2", "ad3", "ad4");

if (in_array($inputUsername, $validUsernames)) {
    // User is valid
    // Now, let's query the 'login' table in the database to validate the user
    $stmt = $conn->prepare("SELECT staffID, password_ FROM administrative_staff WHERE staffID = ?");
    ...
    if ($stmt->num_rows > 0) {
        ...
        echo '<div class="content">';
        echo '<h3>Admin Profile</h3>';
        echo '<p>No Admin found with the provided Admin ID.</p>';
        echo '</div>';
        echo '</div>';
        echo '</body>';
        echo '</html>';
