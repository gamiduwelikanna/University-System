...
if ($checkExistingResult->num_rows > 0) {
    // Student exists, delete the record from the 'stud_contact' table
    $deleteQuery = "DELETE FROM stud_contact WHERE studentID='$studentID'";
    $deleteResult = $connection->query($deleteQuery);

    if ($deleteResult) {
        // Student contact information deleted successfully
        echo '<html>';
        echo '<head>';
        ...
        echo '<h3>Success</h3>';
        echo '<p>Student Contact details deleted</p>';
        echo '</div>';
        echo '</div>';
        echo '</body>';
        echo '</html>';
    } else {
        echo "Error deleting data from stud_contact: " . $connection->error;
    }
} else {
    echo '<html>';
    echo '<head>';
